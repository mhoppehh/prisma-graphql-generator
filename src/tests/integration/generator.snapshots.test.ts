...
const captureGeneratedOutput = () => {
  ...
  return filteredCalls
    .map(call => ({
      filePath: call[0].replace(process.cwd(), '<PROJECT_ROOT>').replace(/\\/g, '/'), // Normalize path separators
      content: call[1]
...
describe('Generator Cartesian Product Snapshot Tests', () => {
  ...
  describe('Core Generator Output Snapshots', () => {
    ...
    describe.each(combinations)('Generator output scenarios', scenarios => {
  ...
  describe('File Merging Scenarios', () => {
    ...
    describe.each(mergingCombinations)('File merging scenarios', scenarios => {
      const scenario = scenarios[0]
...
type Query {
  ...
  describe('Multiple Query/Mutation Combinations', () => {
    ...
    describe.each(multiOpCombinations)('Multiple operation scenarios', combinations => {
